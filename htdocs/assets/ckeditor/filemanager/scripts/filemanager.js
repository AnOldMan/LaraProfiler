...
// We retrieve config settings from filemanager_config_js
var config = (function () {
    var json = null;
    $.ajax({
        'async': false,
        'url': '/admin/filemanager_config_js',
        'dataType': "json",
        cache: false,
...
var fileConnector = '/admin/filemanager';
...
var cleanString = function(str) {
	...
	var cleaned = "";
	var p_search  = 	new Array("Š", "š", "�?", "đ", "Ž", "ž", "Č", "�?", "Ć", "ć", "À",
						"�?", "Â", "Ã", "Ä", "Å", "Æ", "Ç", "È", "É", "Ê", "Ë", "Ì", "�?", "Î", "�?",
	...
	$('#newfolder').unbind().click(function(){
		...
		var getFolderName = function(v, m){
			...
			if(fname != ''){
				...
				foldername = fname.replace(/[^_ a-zA-Z0-9]/g, "");
				foldername = foldername.replace(/[_]+/g, "_");
				if(fname!=foldername){
					alert('Folder name not allowed');
					return;
				}
...
var selectItem = function(data){
	...
	} else if(config.options.domain != false ) {
